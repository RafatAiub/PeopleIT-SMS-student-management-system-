...
- [ ] STUDENT "own library issues" self-service view — `library.routes.ts` reads are currently staff-only (`SUPER_ADMIN, ADMIN, LIBRARIAN`); Phase 0's comments say this was deferred to "Phase 2 scope" but it was never actually built
- [ ] STUDENT/GUARDIAN "own transport assignment" self-service view — same deferral, same gap, `transport.routes.ts`
