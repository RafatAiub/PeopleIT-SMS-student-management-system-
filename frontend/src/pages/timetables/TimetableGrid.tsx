...
const TimetableGrid = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {isAdmin && (
        <div className="glass p-5 rounded-2xl flex flex-wrap items-center gap-6 border border-white/5 bg-slate-900/30">
          <div className="flex flex-col flex-1 min-w-[200px] max-w-xs">
            <label className="text-xs text-slate-400 font-semibold mb-2 uppercase tracking-wider flex items-center gap-1.5">
            ...
            <div className="relative">
              <select
                ...
                className="w-full bg-slate-950/50 border border-slate-700/50 rounded-xl px-4 py-2.5 text-sm font-medium text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/50 appearance-none pr-10"
              >
      ...
      <div className="glass rounded-3xl border border-white/5 overflow-hidden shadow-2xl relative">
        {loading && (
          <div className="absolute inset-0 z-10 bg-slate-950/50 backdrop-blur-sm flex items-center justify-center">
            <div className="text-blue-400 animate-pulse font-semibold">Loading Timetable...</div>
          </div>
        )}
        <div className="overflow-x-auto">
          <div className="min-w-[900px] grid grid-cols-7 divide-x divide-white/5 border-b border-white/5 bg-slate-900/60 text-xs font-bold text-slate-400 uppercase tracking-widest">
          ...
          <div className="min-w-[900px] divide-y divide-white/5">
            {TIME_SLOTS.map(slot => (
              <div key={slot.label} className={`grid grid-cols-7 divide-x divide-white/5 transition-colors hover:bg-white/[0.02] ${slot.isBreak ? 'bg-slate-950/60 text-slate-500 font-semibold' : ''}`}>
                {/* Time Info */}
                <div className="p-4 flex flex-col justify-center items-center bg-slate-900/20">
                  <div className="font-bold text-white text-xs">{slot.label}</div>
                  <div className="text-[10px] text-slate-400 mt-1 font-medium bg-slate-950/50 px-2 py-0.5 rounded-full border border-white/5">{slot.start} - {slot.end}</div>
                </div>
                ...
                {DAYS.map(day => {
                  if (slot.isBreak) {
                    return (
                      <div key={day} className="p-4 flex items-center justify-center text-xs tracking-widest uppercase italic text-slate-500 opacity-50 bg-[repeating-linear-gradient(45deg,transparent,transparent_10px,rgba(255,255,255,0.02)_10px,rgba(255,255,255,0.02)_20px)]">
                        Break
                      </div>
                    );
                  }
                  ...
                  return (
                    <div key={day} className="p-2.5 flex flex-col justify-center min-h-[90px]">
                      {period ? (
                        <div className="h-full w-full rounded-xl bg-gradient-to-br from-blue-500/10 to-indigo-500/5 border border-blue-500/20 shadow-md flex flex-col items-center justify-center p-2 group hover:border-blue-400/30 transition-all">
                          <span className="text-xs font-bold text-blue-300 text-center leading-tight mb-1">{period.subject}</span>
                          
                          {/* Admin/Student shows Teacher name. Teacher shows Class name */}
                          {isTeacher ? (
                            <span className="text-[10px] font-medium text-slate-400 bg-slate-900/80 px-2 py-0.5 rounded-md mt-1 border border-white/5 text-center">
                              {period.className} - {period.sectionName}
                            </span>
                          ) : (
                            <span className="text-[10px] font-medium text-slate-400 bg-slate-900/80 px-2 py-0.5 rounded-md mt-1 border border-white/5 text-center max-w-full truncate">
                              {period.teacher}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="h-full w-full flex items-center justify-center opacity-30">
                          <span className="text-[10px] text-slate-500 text-center italic">Free</span>
                        </div>
                      )}
                    </div>
                  );
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-slate-900 border border-white/10 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden shadow-blue-500/10">
            ...
            <form onSubmit={handleAddSubmit} className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-5">
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Day</label>
                  <select 
                    value={formData.day}
                    onChange={e => setFormData({ ...formData, day: e.target.value })}
                    className="w-full bg-slate-950/50 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                  >
              ...
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full bg-slate-950/50 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/50 placeholder:text-slate-600"
                />
              </div>

              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Teacher Name</label>
                <select 
                  required
              ...
              <div className="pt-6 flex justify-end gap-3 border-t border-white/5">
                <button
                  type="button"
                  onClick={() => setIsAddModalOpen(false)}
                  className="px-5 py-2.5 text-sm font-semibold text-slate-300 hover:text-white hover:bg-white/5 rounded-xl transition-colors"
                >
                  Cancel
                </button>
                <button
