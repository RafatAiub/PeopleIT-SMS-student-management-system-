...
export default function TransportManagement() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Assignments can be fetched if endpoint exists, otherwise empty.
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {routes.filter(r => r.name.toLowerCase().includes(searchQuery.toLowerCase())).map((route) => (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-white/10">
                {vehicles.filter(v => v.registrationNumber.toLowerCase().includes(searchQuery.toLowerCase()) || v.driverName.toLowerCase().includes(searchQuery.toLowerCase())).map((vehicle) => (
        ...
        <div className="glass rounded-2xl border border-white/10 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-white/10">
                {assignments.filter(a => a.studentName.toLowerCase().includes(searchQuery.toLowerCase()) || a.routeName.toLowerCase().includes(searchQuery.toLowerCase())).map((assignment) => (
                  <tr key={assignment.id} className="hover:bg-white/5 transition-colors">
                    <td className="p-4 text-sm font-medium text-white">{assignment.studentName}</td>
                    <td className="p-4 text-sm text-slate-300">{assignment.routeName}</td>
